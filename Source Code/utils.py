...
from keras.layers import GlobalAveragePooling2D, GlobalMaxPooling2D, MaxPooling2D, AveragePooling2D, Flatten
...
def GlobalMaxPool(X):
    model = Sequential()
    model.add(GlobalMaxPooling2D(input_shape=X.shape[1:]))
    X = model.predict(X, batch_size=32)
    return X


def AveragePool(X, pool_size=(2, 2), zero_pad=False):
    model = Sequential()
    if zero_pad:
        model.add(ZeroPadding2D((1, 1), input_shape=X.shape[1:]))
        model.add(AveragePooling2D(pool_size=pool_size))
    else:
        model.add(AveragePooling2D(pool_size=pool_size, input_shape=X.shape[1:]))
    model.add(Flatten())
    X = model.predict(X, batch_size=256)
    return X


def MaxPool(X, pool_size=(2, 2), zero_pad=False):
    model = Sequential()
    if zero_pad:
        model.add(ZeroPadding2D((1, 1), input_shape=X.shape[1:]))
        model.add(MaxPooling2D(pool_size=pool_size))
    else:
        model.add(MaxPooling2D(pool_size=pool_size, input_shape=X.shape[1:]))
    model.add(Flatten())
    X = model.predict(X, batch_size=256)
    return X
